...
| Token Tracking | 입력/출력 토큰 사용량 계산 | O | O | O | O | O | O |
| Latency Analysis | Span별 및 엔드투엔드 Latency 측정 | O | O | O | O | O | O |
| Replay | UI에서 단계별 Trace 재생 | △ | O | △ | O | △ | O |
...
| Retrieval Tracing | 리트리버 쿼리 및 반환된 문서 로깅 | O | O | O | △ | O | O |
...
| Failure Visualization | Trace 내 실패한 단계 강조 표시 | O | O | O | △ | O | O |
...
| Continuous Eval | 예약된 또는 트리거된 Eval 실행 | O | O | O | O | O | O |
| RL/Fine-tuning Link | Fine-tuning 파이프라인과의 연동 | O | △ | △ | △ | △ | △ |

## DevEx / 통합

| 기능 | 설명 | Weave | LangSmith | Langfuse | Braintrust | MLflow | Arize Phoenix |
|---|---|---|---|---|---|---|---|
| SDK 지원 | Python, JS/TS 등 공식 SDK 지원 | O | O | O | O | O | O |
...
| PII Masking | 개인정보(PII) 자동 감지 및 마스킹 | X | △ | O | △ | △ | X |
| Audit Logs | 사용자 및 시스템 작업 감사 로그 | X | O | O | △ | △ | X |
| Data Retention | 구성 가능한 데이터 보관 정책 | X | O | O | O | △ | O |
| Region Support | 멀티 리전 또는 데이터 레지던시 지원 | X | O | O | △ | O | △ |