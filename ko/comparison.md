...
| Hierarchical Spans | 부모-자식 Span 관계 지원 | O | O | O | O | O | O |
| Prompt Logging | LLM Prompt 자동 캡처 | O | O | O | O | O | O |
| Response Logging | LLM 응답 자동 캡처 | O | O | O | O | O | O |
| Token Tracking | 입력/출력 토큰 사용량 집계 | △ | O | O | O | O | O |
| Latency Analysis | Span별 및 전체 구간 Latency 측정 | O | O | O | O | O | O |
| Replay | UI에서 단계별 Trace Replay | △ | O | O | O | △ | O |
...
| Memory Tracing | 대화형 메모리 읽기/쓰기 추적 | X | O | O | △ | △ | O |
| Multi-step Reasoning | 멀티턴 에이전트 추론 체인 시각화 | △ | O | O | O | O | O |
| Workflow Graph | 에이전트 워크플로우의 DAG 또는 그래프 뷰 | O | O | O | O | O | △ |
| Failure Visualization | Trace 내 실패한 단계 강조 표시 | △ | O | △ | O | O | O |
...
| Regression Detection | 품질 저하(Regression) 자동 감지 | △ | O | O | O | O | O |
| Model Comparison | 모델 출력 결과의 병렬 비교 | O | O | O | O | O | O |
| Human Feedback UI | 사람의 어노테이션 및 라벨링을 위한 UI | O | O | O | O | △ | O |

## 모니터링 및 지표 (Metrics)

| 기능 | 설명 | Weave | LangSmith | Langfuse | Braintrust | MLflow | Arize Phoenix |
|---|---|---|---|---|---|---|---|
| Cost Dashboard | 실시간 LLM 비용 추적 Dashboard | O | O | O | O | △ | O |
| Token Analytics | 토큰 사용량 분석 및 트렌드 | △ | O | O | O | O | O |
| Latency Monitoring | Latency 백분위수 및 알림 | O | O | O | O | O | O |
| Error Tracking | 에러율 모니터링 및 알림 | △ | O | O | O | O | O |
| Tool Success Rate | 도구 호출 성공/실패율 | X | O | O | △ | O | O |
...
| Dataset Versioning | Eval 및 학습 데이터셋 버전 관리 | O | O | O | O | O | O |
| Continuous Eval | 예약 또는 트리거 기반 Eval 실행 | O | O | O | O | O | O |
| RL/Fine-tuning Link | Fine-tuning 파이프라인과의 연동 | O | △ | △ | △ | O | △ |
...
| Streaming Tracing | Streaming LLM 응답 Tracing | X | O | O | O | O | O |
| CLI/Infra Integration | CLI 도구 및 Infrastructure-as-code 지원 | △ | O | △ | O | O | O |
...
| RBAC | 역할 기반 액세스 제어 | △ | O | O | O | O | O |
| PII Masking | 개인정보(PII) 자동 감지 및 마스킹 | X | O | O | △ | △ | X |
| Audit Logs | 사용자 및 시스템 작업 감사 로그 | X | O | O | △ | △ | X |
| Data Retention | 구성 가능한 데이터 보존 정책 | X | O | O | O | △ | O |
| Region Support | 멀티 리전 또는 데이터 레지던시 지원 | X | O | △ | △ | O | △ |