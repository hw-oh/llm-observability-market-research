...
| Memory Tracing | Tracking of conversational memory reads/writes | △ | O | O | X | O | △ |
...
| Failure Visualization | Highlighting of failed steps in a trace | O | O | O | O | O | O |
...
| RL/Fine-tuning Link | Integration with fine-tuning pipelines | O | △ | △ | X | O | △ |
...
| Streaming Tracing | Tracing of streaming LLM responses | △ | O | O | O | O | O |
...
| RBAC | Role-based access control | O | O | O | O | O | O |
| PII Masking | Automatic PII detection and redaction | △ | O | O | △ | X | X |
| Audit Logs | Audit trail for user and system actions | △ | O | O | X | X | X |
| Data Retention | Configurable data retention policies | △ | O | O | O | △ | O |
| Region Support | Multi-region or data residency support | △ | O | O | X | △ | △ |
